...
class TestMLModel:
    ...
    def _get_tree_leaf_scores(self, tree):
        """Retrieve a single tree leaf scores for an XGBoost tree
        ...
        if "leaf" in tree:
            ...
            branch_0 = self._get_tree_leaf_scores(tree["children"][0])
            branch_1 = self._get_tree_leaf_scores(tree["children"][1])
    ...
    def _get_min_score_from_XGBRanker(self, ranker):
        ...
        tree_leafs = [
            self._get_tree_leaf_scores(tree) for tree in ranker.get_booster().get_dump()
        ]
